<?php
  function form_insert($table,$form,$titre,$j,$dd){
    ...
    if ($j=="ide") {
     ...
     echo"<form name='frm' action='".$form."$dd$j=sss&' onsubmit='return (conform(this));'  enctype='multipart/form-data'   method='POST'>";
    }
           ...
           elseif($row->type=='string'){
	         ...
	         */
			  elseif($type=="select"){
	            ...
	            echo "<tr BGCOLORe=white><td valign=top> ".str_replace("nom village","Nom Site",str_replace("_"," ",$name)).'</td><td>
	            <select class=mylistkc  id="'.str_replace("_"," ",$name).'" type="text" name="'.$row->name.'" size="1" lange="bonfond:#FFFFFF;bontexte:#400040; erreurfond:#FF0000;bontexte:#0000FF; type:obligatoire;erreur:champs Obligatoire" /<option value=></option>';
				while($ro=mysql_fetch_row($eex)){
					if ($nbt>1) {
  ...
  function insert_table($tab,$form){
    if(isset($_GET["kct"])){
